...
export default function CommunityView() {
  ...
  const [viewMode, setViewMode] = useState<'events' | 'communities'>('events');
      ...
      if (data) {
        ...
        if (!isGlobalMode) {
          filtered = data.filter((c: any) => {
            const uni = c.profiles?.university || 'metu';
            return uni === university || !c.profiles?.university;
          });
        }
          ...
          {isGlobalMode ? (
            ...
            >
              <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                {/* Main Column */}
                <div className="lg:col-span-3">
                  <div className="flex items-center gap-6 border-b-2 border-neutral-200 dark:border-neutral-800 mb-6 font-serif">
                    <button
                      onClick={() => setViewMode('events')}
                      className={`pb-2 text-xl font-bold transition-all relative ${viewMode === 'events' ? 'text-black dark:text-white' : 'text-neutral-400 hover:text-neutral-600'}`}
                    >
                      Etkinlikler
                      {viewMode === 'events' && (
                        <motion.div
                          layoutId="activeTab"
                          className="absolute bottom-[-2px] left-0 right-0 h-1 bg-black dark:bg-white"
                        />
                      )}
                    </button>
                    <button
                      onClick={() => setViewMode('communities')}
                      className={`pb-2 text-xl font-bold transition-all relative ${viewMode === 'communities' ? 'text-black dark:text-white' : 'text-neutral-400 hover:text-neutral-600'}`}
                    >
                      Topluluklar
